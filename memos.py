...
def create(content: str, visibility: str = "PUBLIC") -> Dict[str, Any]:
    """
    Create a new memo with the #AI-Agent tag.
...
# Entry point for skill dispatcher
...
def main(command: str, args: list[str]) -> Dict[str, Any]:
    ...
    try:
        if command == "create":
            if len(args) < 1:
                raise ValueError("create requires at least 1 argument: <content|-|--file path> [visibility]")
            source = args[0]
            rest = args[1:]

            if source == "--file":
                if len(rest) < 1:
                    raise ValueError("--file requires a file path argument")
                file_path = rest[0]
                with open(file_path, "r", encoding="utf-8") as f:
                    content = f.read()
                visibility = rest[1] if len(rest) > 1 else "PUBLIC"
            elif source == "-":
                content = sys.stdin.read()
                visibility = rest[0] if rest else "PUBLIC"
            else:
                content = source
                visibility = rest[0] if rest else "PUBLIC"
